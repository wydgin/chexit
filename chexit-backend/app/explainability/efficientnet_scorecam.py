...
IMG_SIZE = 260
TARGET_IMAGE_NAME = "CHNCXR_0332_1.png"
...
def main():
    ...
    if TARGET_IMAGE_NAME:
        print(f"Targeting specific image: {TARGET_IMAGE_NAME}")
        # We look for the filename within the 'path' column
        df = df[df['path'].str.contains(TARGET_IMAGE_NAME.replace(".png", ""))]
        
        if df.empty:
            print(f"Error: Could not find {TARGET_IMAGE_NAME} in the predictions CSV.")
            return
    else:
        # Original filtering logic only runs if no specific image is targeted
